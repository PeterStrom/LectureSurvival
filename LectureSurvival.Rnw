...
opts_chunk$set(comment=NA, fig.width=6, fig.height=6, dev='pdf', fig.align='center', message=FALSE, warning=FALSE)
...
\subsection{The proprotional hazards assumtion}
\begin{frame}[The proprotional hazards assumtion]
\end{frame}
\end{document}