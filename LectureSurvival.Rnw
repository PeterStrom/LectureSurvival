...
Is 0.17 an extrem value under the null?
...
$$h(t) = \lim_{\Delta t \to 0} \frac{P(t \leq T < t | T \geq t)}{\Delta t}$$
\end{frame}
...
<<Cox1, size='scriptsize'>>=

@

\end{frame}
\begin{frame}[fragile]{Cox regression in R}
<<Cox2, size='scriptsize'>>=

@

\end{frame}
\begin{frame}[fragile]{Cox regression in R}
<<Cox3, size='scriptsize', fig.height=4.5>>=
...
\vspace{0.1in}
<<PropHaz1, size='scriptsize', fig.height=3.5>>=
...
Test if there is a correlation between Shoenfeld residuals and time: 
<<PropHaz2>>=
